...
/**
 * AddProductController class initialized. The AddProductController adds Product data.
 */
public class AddProductController implements Initializable {
    ...
    /**
     * Table column for Product Park (associated part to product) ID.
     */
    public TableColumn idColumnProductPart;
    /**
     * Table column for Product Park (associated part to product) name.
     */
    public TableColumn nameColumnProductPart;
    /**
     * Table column for Product Park (associated part to product) price.
     */
    public TableColumn priceColumnProductPart;
    /**
     * Table column for Product Park (associated part to product) stock.
     */
    ...
    /**
     * Text field for product stock (quantity).
     */
    @FXML
    ...
    /**
     * Text field for part name or part ID.
     */
    @FXML
    public TextField partName_or_ID;

    /**
     * Depending on the data entered by the user, either the product is added upon the user
     * selecting the "save" button (in which they are returned to the Main Screen), or various
     * error messages will pop up, depending on what data is missing or invalid.
     * @param event user selects the "save" button.
     * @throws IOException from FXMLLoader.
     */
    public void clickAddProductSaveButton(ActionEvent event) throws IOException {
        ...
        if ((productName.getText().isEmpty()) || (productPrice.getText().isEmpty()) || (productStock.getText().isEmpty()) || (productMin.getText().isEmpty()) || (productMax.getText().isEmpty())) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Error!");
            alert.setHeaderText("Error: Please ensure all text fields are filled out correctly.");
            alert.setContentText("Once all text fields are filled out, press Save to add product.");
            alert.showAndWait();
        }
        else {
            try {
                ...
                else if ((Integer.parseInt(productStock.getText())) < (Integer.parseInt(productMin.getText())) || (Integer.parseInt(productStock.getText())) > (Integer.parseInt(productMax.getText()))) {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error!");
                ...
                // If no errors occur, then new Product and ProductPart is added.
                Product product = new Product(Inventory.productIndex, productName.getText(), Double.parseDouble(productPrice.getText()), Integer.parseInt(productStock.getText()), Integer.parseInt(productMin.getText()), Integer.parseInt(productMax.getText()));
                ...
                // If Product is successfully added, user is brought back to Main Screen scene.
     ...
     * 3. The part that the user picked is then removed from the product's associated part table.
     * Note: If user presses the Remove Associated Part button before picking a part first, a pop-up
     * error message will display.
     */
    public void clickAssociatedPartRemoveButton(ActionEvent event) {
        ...
        if (part == null) {
            ...
            if (result.get() == ButtonType.OK) {
                // If user confirms by selecting O, the Part is then removed from the product's
    ...
    public void searchBy_PartName_or_ID(ActionEvent event) {
        ...
        // Data in Part table is set.
        tablePart.setItems(part);
    }

    /**
     * Brings user back to the Main Screen scene if they select the cancel button in the Add Product scene.
    ...
    /**
     * AddProductController initialized.
     * @param url url.
